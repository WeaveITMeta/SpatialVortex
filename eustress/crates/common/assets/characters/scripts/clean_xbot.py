"""
Blender script to re-export X-Bot with a clean hierarchy that matches animations.
The goal is to have the armature named "Armature" at the root, so bone paths match.

Run with: blender --background --python clean_xbot.py
"""
import bpy
import os
...
input_path = os.path.join(characters_dir, "x_bot.glb")
output_path = os.path.join(characters_dir, "x_bot_clean.glb")
...
# Import the X-Bot model
print("Importing X-Bot...")
